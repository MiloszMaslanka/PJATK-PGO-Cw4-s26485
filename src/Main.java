public class Main {
    public static void main(String[] args) {
        ZespolProjektowy zespol = new ZespolProjektowy("Campus App");

        Pracownik p1 = new Programista("E-101", "Anna", "Maj", 12000.0, "Java", 4);
        Pracownik p2 = new Tester("E-201", "Piotr", "Lis", 9800.0, true, 25);
        Pracownik p3 = new Programista("E-102", "Karol", "Wojcik", 11500.0, "Kotlin", 3);
        Pracownik p4 = new Tester("E-202", "Marta", "Nowak", 9200.0, false, 18);

        zespol.dodajPracownika(p1);
        zespol.dodajPracownika(p2);
        zespol.dodajPracownika(p3);
        zespol.dodajPracownika(p4);

        zespol.wypiszSkladZespolu();

        System.out.println();
        System.out.println("Łączny koszt zespołu: " + zespol.policzLacznyKoszt() + " zł");

        System.out.println();
        System.out.println("=== Polimorficzne przedstawienie pracowników ===");
        System.out.println(p1.przedstawSie());
        System.out.println(p2.przedstawSie());

        System.out.println();
        System.out.println("Identyfikator pracownika p1: " + p1.pobierzIdPracownika());

        System.out.println();
        System.out.println("=== Bezpieczne rzutowanie z użyciem instanceof ===");
        if (p1 instanceof Programista) {
            Programista programista = (Programista) p1;
            programista.wypiszTechnologie();
        }

        System.out.println();
        zespol.wypiszTesterowAutomatyzujacych();

        System.out.println();
        System.out.println("=== Wyszukiwanie pracownika po ID ===");
        Pracownik znalezionyPracownik = zespol.znajdzPoId("E-201");

        if (znalezionyPracownik != null) {
            System.out.println("Znaleziono: " + znalezionyPracownik);
        } else {
            System.out.println("Nie znaleziono pracownika.");
        }

        System.out.println();
        System.out.println("=== Porównanie obiektów przez equals() ===");
        System.out.println(
                p2.equals(new Tester("E-201", "Inne", "Dane", 9000.0, false, 10))
        );

        System.out.println();
        System.out.println("=== Działanie toString() ===");
        System.out.println(p2);
    }
}